...
def sp(file,keyword):
    arr = readf(file)
    idx, name, key, s, l, x, y, z, theta, phi, psi, sx, sy, sz = search_engine(arr,keyword)
    
    plt.figure(figsize=(10, 8))
    plt.plot(s,sx,'b-',label = r'$S_x$')
    plt.plot(s,sy,'g-',label = r'$S_y$')
    plt.plot(s,sz,'r-',label = r'$S_z$')

    plt.xlabel('s(m)',size = 20)
    plt.ylabel('Spin Component',size = 20)
    plt.title(r'Spin Motion of $e ^ - %s$ (Co-Moving Frame)',size =20)
